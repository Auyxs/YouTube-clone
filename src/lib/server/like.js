...
export async function addLike(userId, videoId) {
  const likeData = await readFromFile(LIKE_DATA_FILE);
  if (likeData.find((like) => like.userId == userId && like.videoId == videoId))
    return;
...
export async function removeLike(userId, videoId) {
  const likeData = await readFromFile(LIKE_DATA_FILE);
  if (
    !likeData.find((like) => like.userId == userId && like.videoId == videoId)
  )
    return;
  const updatedLikes = likeData.filter(
    (like) => like.userId !== +userId || like.videoId !== +videoId
  );
